...
class CoreLib extends TagLib {
    constructor(options, renderer) {
        ...
        this.tagMap = {
            ...
            'param':       (...params) => self.tagParam.apply(self, params)
        };
    ...
    tagParam(node) {
        return `${node.token.params.name}=${encodeURIComponent(node.token.params.value)}`;
    }
    ...
    tagUrl(node, index, data) {
        console.log(node);
        let value = node.token.params.value;

        if (node.children && node.children.length > 0) {
            const paramArray = [];
            for (let children of node.children) {
                if (children.type === 'tag') {
                    paramArray.push(this.tagParam(children));
                }
            }
        ...
        if ('var' in node.token.params) {
            data[node.token.params.var] = value;
            return this.wrapReturn('', data);
        }
